...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "blaze-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">blazecodeprakhar@github: ~$ ./portrait.sh</text>')
...
status_text = "blazecodeprakhar@github:~$ whoami Prakhar Yadav"
cursor_x = PAD + int(len(status_text) * 7.8)
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'blazecodeprakhar@github:~$ whoami <tspan fill="{INK}">Prakhar Yadav</tspan></text>')
